#include "include.h"
extern u8 rx_buf[32];	
extern int16_t offset_mx,offset_my,offset_mz;	
...
uint16_t VirtAddVarTab[NumbOfVar] = {0xAA00, 0xAA01, 0xAA02,0xAA03,0xAA04,0xAA05,0xAA06,0xAA07,0xAA08};
...
static void EE_READ_GY_OFFSET(void)
{
	EE_ReadVariable(VirtAddVarTab[3], &sensor.gyro.quiet.x);
	EE_ReadVariable(VirtAddVarTab[4], &sensor.gyro.quiet.y);
	EE_ReadVariable(VirtAddVarTab[5], &sensor.gyro.quiet.z);
}

void EE_SAVE_GY_OFFSET(void)
{
  EE_WriteVariable(VirtAddVarTab[3],sensor.gyro.quiet.x);
  EE_WriteVariable(VirtAddVarTab[4],sensor.gyro.quiet.y);
	EE_WriteVariable(VirtAddVarTab[5],sensor.gyro.quiet.z);
}	
static void EE_READ_MG_OFFSET(void)
{
	EE_ReadVariable(VirtAddVarTab[6], &offset_mx);
	EE_ReadVariable(VirtAddVarTab[7], &offset_my);
	EE_ReadVariable(VirtAddVarTab[8], &offset_mz);
}

void EE_SAVE_MG_OFFSET(void)
{
  EE_WriteVariable(VirtAddVarTab[6],offset_mx);
  EE_WriteVariable(VirtAddVarTab[7],offset_my);
	EE_WriteVariable(VirtAddVarTab[8],offset_mz);
}	
...
void	paramLoad(void)
{
	...
	EE_READ_GY_OFFSET();  //采集陀螺仪零偏
	EE_READ_MG_OFFSET();
 ctrl.pitch.shell.kp = 0.9 ; //0.9   
	ctrl.pitch.shell.ki = 0.008;//0.008
	ctrl.pitch.shell.kd = 0.010; //0.010   
//	
	ctrl.pitch.core.kp = 0.8;   //0.8
	ctrl.pitch.core.kd = 0.15;//0.15
/*	ctrl.pitch.shell.kp = 2;    
//	ctrl.pitch.shell.ki = 0.01;
//	ctrl.pitch.shell.kd = 0.6;    //2
//	ctrl.pitch.core.kp = 1.8;   //1.5
//	ctrl.pitch.core.kd = 0.22;  */ //0.16
//	
//	//The data of roll
	ctrl.roll.shell.kp = 0.9;//0.9
	ctrl.roll.shell.ki = 0.008;//0.008
	ctrl.roll.shell.kd = 0.010;//0.010

	ctrl.roll.core.kp = 0.8;//0.8
	ctrl.roll.core.kd = 0.15;//0.15
///*	ctrl.roll.shell.kp = 2;
//	ctrl.roll.shell.ki = 0.01;
//	ctrl.roll.shell.kd = 0.6;
//	ctrl.roll.core.kp = 1.8;
...
//	
//	//The data of yaw
	ctrl.yaw.shell.kp =1;//1
	ctrl.yaw.shell.kd = 0.01;//0.01
//	
	ctrl.yaw.core.kp =0.8;//0.8
	ctrl.yaw.core.kd = 0.06;//0.06
	Rc_Data.pitch_offset = 1500;
	Rc_Data.roll_offset = 1500;
	Rc_Data.yaw_offset = 1500;

	Rc_Data.ROLL=Rc_Data.roll_offset;
	Rc_Data.PITCH=Rc_Data.pitch_offset;
	Rc_Data.YAW =Rc_Data.yaw_offset;
