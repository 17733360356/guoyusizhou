...
extern u16 US_100_DATA;	
State Init_State;
float BATT[20];
extern float THR_Lock;
extern float US100_Alt;
void IAC_Init(void)
{
	 delay_init(72);  //延时函数初始化
	 Nvic_Init();     //中断初始化 
		uart_init(500000);								//uart init
	 LED_Init();      //led初始化
	 OLED_Init();     //oled初始化
	 Draw_LibLogo();  
	 KEY_Init();      //按键初始化
...
void  BATTDispaly(void)
{
  ...
  Dis_Float(4,37,Battery_voltage,1);			//计算电池电压
   ...
   Dis_Float(2,5,(double)US100_Alt*10000,1);
	   Dis_Float(2,55,(double)THR_Lock,1);
